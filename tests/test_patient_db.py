...
class TestIntegration:
    ...
    def test_full_workflow(self, db, temp_csv_file):
        ...
        # Simulate receiving ADT message (new patient)
        new_patient = PatientInfo(mrn="100000001", age=35, sex="F")
        db.upsert_patient(new_patient)

        # Simulate receiving ORU message (new creatinine)
        ...
        assert patient["age"] == 35  # Updated from ADT
        assert patient["sex"] == "F"
        assert len(patient["creatinine_history"]) == 3  # 2 from CSV + 1 from ORU
